...
class TokenDTO {
  ...
  TokenDTO({required String token}) {
    this._token = token;
  }

  TokenModel token() {
    var json = jsonDecode(this._token);
    _setAccessToken(json[0]['access_token']);
  ...
  _setAccessToken(String value) {
    this._access_token = value;
  }

  _setRefreshToken({required int id, required int user_id, required String token, required String expires_in, required String timestamp}) {
...
void main(List<String> args) {
  ...
  final tokenDto = TokenDTO(token: token_text);

  var token = tokenDto.token();

  var access = token.access_token;
  print(access);
  var refresh = token.refresh_token;
  print(refresh.id);
}
/**
